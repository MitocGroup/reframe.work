...
function render_article ($layout_classes, $layout_styles, $layout_sections, $layout_containers, $layout_objects, $divide_second, $postNumber, $count) {
    if ( $divide_second === 'true' ? true : false ) {
        $imgStatus = 'articleInline';
        $isLast = $count===$postNumber;
        $isFirst = $postNumber%3 === 1;
        $isSecond = $postNumber%3 === 2;

        if ( ($isSecond && $isLast) || $isFirst ) {
            $imgStatus = 'articleBlock';
        }
        $layout_classes[] = $imgStatus;
    }    

    $content = '<article '.render_element_class($layout_classes).'>'."\n";
