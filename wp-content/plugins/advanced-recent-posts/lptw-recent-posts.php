...
function lptw_display_recent_posts ( $atts ) {
    ...
    if( $lptw_shortcode_query->have_posts() ) {
        ...
        while( $lptw_shortcode_query->have_posts() ) {
            ...
            if ($a['layout'] == 'basic' ) {
                if ($url != '') {
                    ...
                    $img_content = '<div style="background: transparent url('.$url.') no-repeat content-box center" alt="'.get_the_title().'" class="thmbImageFluid"></div>';
                ...
                $content .= render_article ($layout_classes, '', $layout_sections, $layout_containers, $layout_objects, $divide_second, $postNumber, $count);
            ...
            /* --------- Responsive Grid - recent posts with thumbnail and featured posts --------- */
                ...
                if ($featured == 'on') {
                    ...
                    if ($embedded_video == 'on' && $embedded_video_frame !== false) {
                        ...
                        $img_tag = 'div';
                        $img_class = 'imageContainer';
                        $img_content = '<a href="'.get_the_permalink().'"><div class="thmbImage" style="background: transparent url('.$thumb_grid['0'].') no-repeat content-box center; background-size:cover";  alt="'.get_the_title().'"></div></a>';
                    }
                    /* image end */
        ...
        $content .= '</div>';
        if ($a['layout'] == 'grid-medium-DISABLED') {
            $content .= '<script>
